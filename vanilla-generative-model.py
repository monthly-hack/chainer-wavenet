...
x_sample = Variable(cuda.cupy.zeros((1, 1, gen_len, 1), dtype=np.float32), volatile=True)

for loc in range(x_sample.data.shape[2])[1024:]:
    prob = F.softmax(model.predictor(x_sample[:,:,loc-1024:loc])[-1:], use_cudnn=False).data
    prob = cuda.to_cpu(prob)[0]
    x_sample.data[0, 0, loc] = np.random.choice(u, p=prob)
    if loc%200==0:
        print(loc)

wavfile.write('result.wav', 8000, (cuda.to_cpu(x_sample.data) * scale).astype(np.int16).reshape((-1,)))
